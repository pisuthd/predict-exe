      ...
      <div className="max-w-7xl mx-auto px-4 py-8 relative">
              ...
              <div>
                ...
                <p className="text-gray-400 text-sm mt-1">
                  {account 
                    ...
                    : 'Public round history - Connect wallet to see your bets'}
                </p>
              </div> 
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
            <div className="bg-black border-2 border-green-500/50 p-4">
              <div className="text-center">
                ...
                <div className="text-2xl font-bold text-green-500">{userStats.winRate.toFixed(1)}%</div>
            ...
            <div className="bg-black border-2 border-purple-500/50 p-4">
              <div className="text-center">
                ...
                <div className="text-2xl font-bold text-purple-500">{userStats.totalBets}</div>
            ...
            <div className="bg-black border-2 border-yellow-500/50 p-4">
              <div className="text-center">
                ...
                <div className={`text-2xl font-bold ${userStats.netProfit >= 0 ? 'text-green-500' : 'text-red-500'}`}>
        ...
        <div className="bg-black border-2 border-gray-500/50 mb-6">
          <div className="flex space-x-0 border-b border-gray-500/50 overflow-x-auto">
            <button
              ...
              className={`px-6 py-3 font-bold text-sm tracking-wider transition-colors whitespace-nowrap ${
                filter === 'all'
            ...
            >
              ALL ROUNDS
              <span className="ml-2 text-xs">({allRounds.length})</span>
            </button>
            {account && (
              <>
                <button
        ...
        {(loadingHistory || isLoading) && (
          <div className="bg-black border-2 border-cyan-500/50 p-8 text-center">
            <div className="text-cyan-500 text-lg">Loading history...</div>
          </div>
        )}

        {/* Empty State */}
        {!loadingHistory && !isLoading && filteredHistory.length === 0 && (
          <div className="bg-black border-2 border-gray-500/50 p-12 text-center">
              ...
              <table className="w-full text-sm">
                ...
                <tbody>
                  {filteredHistory.map((round) => {
                    ...
                    const canClaim = account && userWon && payout > 0; // Simplified - you'd check if already claimed
                    
                    const upOdds = calculateOddsFromPool(round, 'UP');
                    const downOdds = calculateOddsFromPool(round, 'DOWN');

                    return (
                      <tr key={round.roundId} className={`border-b border-gray-800/50 hover:bg-gray-800/30 transition-colors ${hasUserBet ? 'bg-blue-500/5' : ''}`}>
                        <td className="p-4 text-cyan-500 font-bold">#{round.roundId}</td>
                        ...
                        <td className="p-4">
                          <div>
                            ...
                            <div className={`text-sm font-bold ${parseFloat(priceChange) >= 0 ? 'text-green-500' : 'text-red-500'}`}>
                              {parseFloat(priceChange) >= 0 ? '+' : ''}{priceChange}%
                            </div>
                          </div>
                        </td>
                        <td className="p-4">
                          <div className={`flex items-center space-x-2 font-bold ${round.upWins ? 'text-green-500' : 'text-red-500'}`}>
                            {round.upWins ? <TrendingUp className="w-5 h-5" /> : <TrendingDown className="w-5 h-5" />}
                            <span>{round.upWins ? 'UP' : 'DOWN'}</span>
                          </div>
                        </td>
                        <td className="p-4">
                          <div className="text-white font-bold">{round.totalPool.toFixed(2)} MAS</div>
                        ...
                        <td className="p-4">
                          <div className="text-xs space-y-1">
                            <div className={round.upWins ? 'text-green-500 font-bold' : 'text-gray-500'}>
                              UP: {upOdds.toFixed(2)}x
                            </div>
                            <div className={!round.upWins ? 'text-red-500 font-bold' : 'text-gray-500'}>
                          ...
                          <td className="p-4">
                            {canClaim ? (
                              <button
                                ...
                                className="flex items-center space-x-1 px-3 py-2 border border-green-500 text-green-500 hover:bg-green-500/20 transition-colors text-xs font-bold disabled:opacity-50 whitespace-nowrap"
                              >
        ...
        {!account && allRounds.length > 0 && (
          <div className="mt-8 bg-black border-2 border-purple-500/50 p-6">
            <div className="flex items-center justify-between">
              <div className="flex items-center space-x-4">
                <Lock className="w-8 h-8 text-purple-500" />
                <div>
                  <h3 className="text-purple-500 font-bold text-lg">CONNECT WALLET</h3>
                  <p className="text-gray-400 text-sm">Track your bets and claim winnings</p>
                </div>
              </div>
              <button className="px-6 py-3 border-2 border-purple-500 text-purple-500 hover:bg-purple-500/20 font-bold transition-colors">
                CONNECT WALLET
              </button>
            </div>
          </div>
